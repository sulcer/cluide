...
/* Custom variants: shadcn's `init` writes these to map data-open/data-closed/data-checked/
   data-unchecked/data-selected/data-disabled/data-active/data-horizontal/data-vertical to
   Radix's data-state (and data-orientation) attributes. This repo never ran `init` (Step 7 hand-writes
   components.json instead), so paste them by hand or every generated component's state classes
   (switch colors, dialog/sheet enter-exit animations) are dead. */
...
If the dry run rejects `radix-nova`, set `"style": "new-york"` (the documented Radix style) and retry. After the add, open `package.json`: every range the CLI added (`^`, `~`) becomes the exact version already installed (`bun pm ls` shows it), then `bun install`. If the add touched `src/globals.css`, restore it to Step 6. `init` was not run, so the custom variants it writes are pasted by hand (Step 6) rather than generated.
...
In `src/components/ui/dialog.tsx` change the overlay's `bg-black/50` to `bg-black/45`; in `src/components/ui/sheet.tsx` to `bg-black/30` (`tokens.md` Overlays; the command menu shares the dialog's overlay, 0.05 off the spec and invisible). Also in `sheet.tsx`, remove `SheetContent`'s per-side `data-[side=left]:sm:max-w-sm`/`data-[side=right]:sm:max-w-sm` caps from the generated content class string — their attribute-qualified selector outranks a width passed through `className` (e.g. `sm:max-w-[520px]`), so a caller can never widen a sheet past 384px otherwise; the per-side width stays each caller's job. Leave the rest of the generated files alone.
          ...
          <CommandList className="max-h-[380px] p-1">
            ...
            {recent.length > 0 && (
              <Group heading="Open file">
                {recent.map((r) => (
                  <Item key={r.path} icon={<History className="size-4" />} label={r.label} hint={r.path} onSelect={() => go(r.url)} />
                ))}
              </Group>
            )}
...
test("mcp-project with approval, shadowing and the sheet", async ({ page }) => {
  ...
  // aria-hidden, so getByRole("status") finds nothing even though the toast is visible. A plain
  // "Saved" text match is ambiguous too: the sheet's own now-clean SaveBar reads "Saved" as well.
  // Match the toast's DOM attribute directly, which — unlike getByRole — isn't filtered by
  // aria-hidden.

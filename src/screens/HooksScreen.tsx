...
import { scopeUrl, screenUrl } from "@/lib/routes";
...
export function HooksScreen({ scope }: { scope: Scope }) {
  ...
  const failed = [settings, local, scripts].find(
    (r) => r.error !== undefined && r.error.code !== "offline" && r.data === undefined,
  );
  ...
  const settingsLink = (text: string) => (
    <Link to={screenUrl(scope, "settings")} className="text-link underline underline-offset-2">
      {text}
    </Link>
  );
                ...
                return (
                  <tr key={i} className="h-9 border-b hover:bg-accent">
                    ...
                    <td className="px-4 font-mono text-xs">
                      {script ? (
                        <Link
                          to={scriptUrl(script.name)}
                          title={r.command}
                          className="inline-flex max-w-[520px] items-center gap-1 text-link"
                        >
                          <span className="min-w-0 truncate">{r.command}</span>
                          <ArrowRight className="size-3.5 shrink-0" />
                        </Link>
                      ) : (
                        <span title={r.command} className="block max-w-[520px] truncate">
                          {r.command}
                        </span>
                      )}
                    </td>
        ...
        <div className="max-w-[480px] px-4 pt-5 pb-4">
          ...
          {scripts.data.map((s) => (
            <Link
              key={s.path}
              to={scriptUrl(s.name)}
              className="flex h-7 items-center gap-2 rounded-sm px-1 hover:bg-accent"
            >
